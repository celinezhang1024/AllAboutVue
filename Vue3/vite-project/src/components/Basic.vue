...
<template>
  <div>
      ...
      <div>4. {{str.split(',').map(v => `<div>${v}</div>`)}} - map方法</div>
      <div>5. <span v-text="str"></span> - v-text包裹文本</div>
      <div v-html="hl"></div>
      ...
      <div>16. 查看源码 <span :key="index" v-for="(item,index) in aRR">{{item}},</span></div>
    </div>
  </template>
